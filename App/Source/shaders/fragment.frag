...
vec4 background(vec3 ray)
{
    ...
    vec4 nebulae = texture(albedo, (uv*1.5 ));
...
vec4 raymarchDisk(vec3 ray, vec3 zeroPos)
{
    ...
    if (!disk) return vec4(1.,1.,1.,0.); //no disk
...
void main()
{
    ...
    for( int i=0; i<AA; i++ )
    {
        ...
        vec3 pos = vec3(0.,0.05,-(20.*mouse.xy/resolution.y-10.)*(20.*mouse.xy/resolution.y-10.)*.05); 
        vec2 angle = vec2(time*0.1,.2);      
        angle.y = (2.*mouse.y/resolution.y)*3.14 + 0.1 + 3.14;
